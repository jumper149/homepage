...
One thing you can't avoid is choosing the order of your transformer stack.
When everything is just a `ReaderT`, this doesn't matter, but be careful, when you set up a transformer stack with monadic state.
...
instance MonadTransControl AppT where
  ...
  liftWith f = AppT $ f $ liftWith . unAppT
  restoreT = AppT . restoreT
----
...
TODO: typeclass `MonadConfiguration`
  ...
  runStdOutLogging $ do
    ...
    (\ tma -> runReaderT tma config) $ do
      runDatabaseT config $ do
        lift . lift $ $logInfo $ "Connected to database."
        unAppT app
...
===== ``AppT``'s instances
To use the environment you will have to provide a few class instances.
...
instance (Monad m) => MonadReader Configuration (AppT m) where
  ask = AppT . lift . queryDatabase
  local = undefined -- TODO: Implement using `MonadTransControl`.
